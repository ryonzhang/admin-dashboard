...
export const UserInputGroup: FunctionComponent<UserInputGroupProps> = ({values,errors,deleteable,namePrefix,onDelete,touched,disabledFields}) =>{
    ...
    return <div className='user-input-group'>
        <InputField className='user-group-item' labelTextID={TEXT_ID.FIRST_NAME} error={errors && errors.firstName} touched={touched && touched.firstName} disabled={!!disabledFields && disabledFields.includes(FIELDS.FIRST_NAME)} name={getPrefixedName(namePrefix,'firstName')}  placeholderTextID={TEXT_ID.FIRST_NAME} type={'string'} value={values.firstName} />
        <InputField className='user-group-item' labelTextID={TEXT_ID.LAST_NAME} error={errors && errors.lastName} touched={touched && touched.lastName} disabled={!!disabledFields && disabledFields.includes(FIELDS.LAST_NAME)} name={getPrefixedName(namePrefix,'lastName')}  placeholderTextID={TEXT_ID.LAST_NAME} type={'string'} value={values.lastName} />
        <InputField className='user-group-item' labelTextID={TEXT_ID.EMAIL} error={errors && errors.email} touched={touched && touched.email} disabled={!!disabledFields && disabledFields.includes(FIELDS.EMAIL)} name={getPrefixedName(namePrefix,'email')}  placeholderTextID={TEXT_ID.EMAIL} type={'string'} value={values.email} />
        <Can
            role={authUtils.getRole()}
            perform='assign:juvo-role'
            yes={()=>(
                <InputField className='user-group-item' options={departmentOptionArrayForJuvo} labelTextID={TEXT_ID.DEPARTMENT} error={errors && errors.department} touched={touched && touched.department} disabled={!!disabledFields && disabledFields.includes(FIELDS.DEPARTMENT)} isDropdown name={getPrefixedName(namePrefix,'department')}  placeholderTextID={TEXT_ID.PLEASE_SELECT_AN_OPTION} type={'number'} value={values.department && getOption(departmentOptionArrayForJuvo,values.department)} />
            )}
            no={()=>(
